...
def generate_dataset(X, tokenizer, pad_to_max_length=True, add_special_tokens=True, max_length=256, return_attention_mask=True, return_tensors='pt'):
    tokens_query = tokenizer.batch_encode_plus(
        ...
        # 0: padded tokens, 1: not padded tokens; taking into account the sequence length
...
def pool_summary(last_hidden_states, pool_summary_op="max"):
    ...
    last_hidden_states_p = last_hidden_states.permute(0, 2, 1)
    func_pool_summmary = F.max_pool1d if pool_summary_op == "max" else F.avg_pool1d
    # [batch_size, num_features]
    return func_pool_summmary(last_hidden_states_p, kernel_size=num_features).squeeze(-1)
...
def compute_bert_representations(input_file, output_file, col_id, col_targets, tokenizer, model, max_length=256, batch_size=128, device="cuda:0"):
    df = pd.read_csv(input_file)
    df.fillna('', inplace=True)
    if col_id not in df:
        df[col_id] = df[col_targets[0]].to_list()

    df["target"] = ""
    for col_t in col_targets:
        df["target"] += df[col_t].map(str) + " "
...
def main():
    ...
    parser.add_argument("--input_queries_path_file", type=str,
                        default=None, help="Input CSV with the query information.")
    parser.add_argument("--output_queries_path_file", type=str, default=None,
                        help="Output file with the mapping of the queries to BERT representations.")
    parser.add_argument("--input_product_catalogue_path_file", type=str,
                        default=None, help="Input CSV with the product information.")
    parser.add_argument("--output_product_catalogue_path_file", type=str, default=None,
                        help="Output file with the mapping of the queries to BERT representations.")
    parser.add_argument("--model_name", type=str,
                        default="bert-base-multilingual-uncased", help="BERT multilingual model name.")
    parser.add_argument("--bert_max_length", type=int, default=256,
                        help="Tokens consumed by BERT (512 tokens max).")
    ...
    col_targets = ["product_title", "product_brand", "product_color_name"]
